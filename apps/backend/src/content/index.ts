...
const createContentModule = ( { api, socketServer }: ContentModuleOptions ) => {
  ...
  socketServer.on( 'content:shabad:open', setShabad )
  // socketServer.on( 'content:shabad:open-next', setNextShabad )
  // socketServer.on( 'content:shabad:open-previous', setPreviousShabad )
  // socketServer.on( 'content:bookmark:open', setBookmark )
