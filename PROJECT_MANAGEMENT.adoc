...
- https://github.com/ShabadOS/{repo}/releases[Release] - built on merging to master branch, versioning based on commit history (can include alpha and beta)

When merging into `master`, ensure that there is a commit with the appropriate SemVer bump. This will build and publish Shabad OS of that version to GitHub releases.
